"""
Seed script to populate PRODUCTION database with sample categories and products
...
def seed_products(db: Session, category_map: dict):
    ...
    for prod_data in products_data:
        ...
        if not category_id:
            print(f"⚠️  Warning: Category '{category_slug}' not found, skipping product '{prod_data['name']}'")
            skipped_count += 1
            continue
...
def main():
    ...
    print("Database: fastapi_api @ localhost:5432")
    print("=" * 60)
    ...
    try:
        ...
        print(f"✅ Categories processed! ({len(category_map)} total)")
        
        # Seed products
        print("\n🛒 Seeding products...")
        created, skipped = seed_products(db, category_map)
        print(f"✅ Products processed! ({created} created, {skipped} skipped)")
        ...
        print("=" * 60)
        print("\n🌐 Production API: https://fastapi.kevinlinportfolio.com")
        print("📖 View products: https://fastapi.kevinlinportfolio.com/api/v1/products/")
        print("=" * 60)
